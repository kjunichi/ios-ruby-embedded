...
task "MRuby.framework/Versions/1.0.0/" => "MRuby.framework/Versions" do
  mkdir "MRuby.framework/Versions/1.0.0/Resources"
...
task :build_mruby => "ios_build_config.rb" do
  cp "custom.gembox", "mruby/mrbgems/"
  Dir.chdir("mruby") do
...
file "bin/mirb" => [:build_mruby, "bin"] do
  cp "mruby/build/host/bin/mirb", "bin/mirb"
end

file "bin/mrbc" => [:build_mruby, "bin"] do
  cp "mruby/build/host/bin/mrbc", "bin/mrbc"
end

file "bin/mruby" => [:build_mruby, "bin"] do
  cp "mruby/build/host/bin/mruby", "bin/mruby"
end
...
task :mruby_headers => [:build_mruby, "MRuby.framework/Versions/1.0.0/Headers"] do
  cp_r "mruby/include/.", "MRuby.framework/Versions/Current/Headers/"
...
task :all => [:verify_sysroot, "mruby", "bin/mirb", "bin/mrbc", "bin/mruby", "MRuby.framework/Versions/Current/MRuby", :mruby_headers]
...
task :clean => "ios_build_config.rb" do
  ...
  rm_f ["ios_build_config.rb", "bin/mirb", "bin/mrbc", "bin/mruby", "mruby/mrbgems/custom.gembox"]
  rm_rf "MRuby.framework"
end
